...
export default function AdminBannerTwo() {
  ...
  const fetchBanners = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/banner-two`);
      setBanners(res.data);
    } catch (err) {
      console.error("Error fetching banners:", err);
      alert("Failed to fetch banners from server.");
    }
  };
  ...
  const saveBanner = async () => {
    ...
    try {
      if (editId) {
        // UPDATE
        await axios.put(`${BASE_URL}/banner-two/${editId}`, form);
      } else {
        // ADD NEW
        await axios.post(`${BASE_URL}/banner-two`, form);
      }
  ...
  const deleteBanner = async (id) => {
    ...
    try {
      await axios.delete(`${BASE_URL}/banner-two/${id}`);
      fetchBanners();
    } catch (err) {
