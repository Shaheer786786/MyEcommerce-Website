...
const BASE_URL = "https://my-backend-93up.onrender.com"; // Optional: backend base URL
          ...
          <div className="co-summary-items">
            {orderItems.map(item => (
              <div key={item.id} className="co-summary-item">
                <div className="co-product-name">
                  <img
                    src={item.images?.[0]?.startsWith("http") ? item.images[0] : `${BASE_URL}/images/${item.images?.[0] || item.image || "default-product.png"}`}
                    alt={item.name}
                  />
                  <span>{item.name}</span>
                </div>
