...
function ProductDetail({ addToCart }) {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      try {
        ...
        const found = merged.find((p) => p.id === Number(id) || p.id === id);
        if (found) setProduct(found);
        else setError(true);
      } catch (err) {
        console.error(err);
  ...
  const images =
    product.images?.length > 0
      ? product.images.map((img) =>
          img.startsWith("http") ? img : `http://127.0.0.1:5000/images/${img}`
        )
      : product.image
      ? [product.image.startsWith("http") ? product.image : `http://127.0.0.1:5000/images/${product.image}`]
      : ["https://via.placeholder.com/400"];
  ...
  return (
    <div className="pdx-wrapper">
      {/* Product Card, Images, Info, Highlights, Variants, Specifications, Reviews, FAQs */}
      {/* Toasts */}
      {/* Related Products, Ads, Social Media */}
      {/* This part includes all your HTML structure exactly as in your code */}
    </div>
  );
}
