...
function Products({ searchQuery = "", addToCart }) {
  ...
  if (loading) return <h3 className="prd-loading-text">Loading products...</h3>;
        ...
        {filteredProducts.map((product, index) => {
          ...
          return (
            ...
            >
              ...
              <div className="prd-product-info">
                <h4 className="prd-product-title">{product.name}</h4>
                <p className="prd-product-short-desc">{product.shortDesc || "High performance with premium quality"}</p>

                <div className="prd-rating-offer-row">
                  <div className="prd-rating-box">
                    ⭐ {product.rating || 4.3} ({product.reviews || 0} Reviews)
                    <br />
                    <span className="prd-stock-text">{isOutOfStock ? "Out of Stock" : `Stock: ${product.stock}`}</span>
                  </div>
                  {product.offer && <span className="prd-inline-offer">{product.offer}</span>}
                </div>
                ...
                <div className="prd-action-buttons" onClick={(e) => e.stopPropagation()}>
                  <button className="prd-buy-btn" disabled={isOutOfStock} onClick={() => handleBuyNow(product)}>Buy Now</button>
                  <button className="prd-cart-btn" disabled={isOutOfStock} onClick={() => handleAddToCart(product)}>Add to Cart</button>
                </div>
              </div>
            </div>
          );
